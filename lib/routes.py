...
from lib.common import (
    xbmc, xbmcgui, xbmcplugin, xbmcaddon, handle, profile, translate,
    addMenuItem, setcontent, end, setview,
    notify, exists, log, go_home, quote_plus,
)
...
def build_series_playlist(imdb_number, season_num, current_episode_num, serie_name, original_name, all_episodes):
    ...
    from lib.common import urlencode
...
def live_categories():
    ...
    if cat:
        ...
        if exists(IPTV_PROBLEM_LOG):
            with open(IPTV_PROBLEM_LOG, "r", encoding='utf-8') as arquivo:
                if url_problem in arquivo.read():
                    check = True
        with open(IPTV_PROBLEM_LOG, "a", encoding='utf-8') as arquivo:
            if not check:
...
def open_channels(param):
    ...
    open_epg(header='TV AO VIVO', channels=channels, build_listitem=build)
...
def channels_pluto():
    ...
    open_epg(header='PLUTO TV', channels=pluto.to_lazy_channels(channels), build_listitem=build)
...
def imdb_movies():
    addMenuItem({'name': 'PESQUISAR FILME', 'description': ''}, destiny='/find_movies')
...
def imdb_series():
    addMenuItem({'name': 'PESQUISAR SÉRIE', 'description': ''}, destiny='/find_series')
    addMenuItem({'name': 'TOP 250 SÉRIES', 'description': ''}, destiny='/imdb_series_250')
    addMenuItem({'name': 'SÉRIES POPULARES', 'description': ''}, destiny='/imdb_series_popular')
    end()
    setview('WideList')
...
def find_movie_in_list(active, movie_name, original_name, year):
    from lib import xtream, matcher
    dns = active['dns']
    username = active['username']
    password = active['password']
    ...
    if not match:
        ...
        return None
    stream_url = xtream.API(dns, username, password).movie_play_url(
...
def play_resolve_series(param):
    ...
    try:
        ...
        if resume_data and resume_data[0] > 0:
            from lib.common import ask_resume
...
def router(action, params):
    if action == 'select_list':
        ...
        play_radio(params)
    elif action == 'imdb_movies':
        imdb_movies()
    elif action == 'imdb_series':
        imdb_series()
